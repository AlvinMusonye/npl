...
export default function TransactionRecordsPage({ setRole }) {
    ...
    useEffect(() => {
        // Pre-fill the sender with the current user's ID from localStorage
        const user = JSON.parse(localStorage.getItem('user'));
        if (user && user.id) {
            setFormData(prev => ({ ...prev, sender: user.id }));
        }
    }, []);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setFormData({
                transaction_id: '', sender: formData.sender, receiver: '', amount: '',
                ...
                <main className="flex-1 p-4 sm:p-6 lg:p-8 lg:ml-80 lg:mr-0 mr-20 transition-all duration-300">
                    <div className="p-0 lg:p-4 w-full">
                        ...
                        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-6">
                            <InputField icon={Hash} label="Bank/Payment Reference ID" name="transaction_id" placeholder="e.g., BUYER_PAYMENT_REF_45678" value={formData.transaction_id} onChange={handleChange} />
                            <InputField icon={User} label="Receiver (Lender's User ID)" name="receiver" placeholder="Enter lender's user UUID" value={formData.receiver} onChange={handleChange} />
                            <InputField icon={DollarSign} label="Amount" name="amount" type="number" placeholder="e.g., 1200000" value={formData.amount} onChange={handleChange} />
