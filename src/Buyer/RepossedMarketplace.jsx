      ...
      <ProductCard className="group">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="flex items-start gap-2">
            ...
            <h3 className="font-medium text-gray-900 leading-tight line-clamp-2" title={asset.collateral_identifier}>
              {asset.collateral_identifier || 'Repossessed Asset'}
            </h3>
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="text-xl font-semibold text-gray-900">
              KSh {Number(asset.sale_price_kes).toLocaleString()}
            </div>
          </div>

          {/* Action Button */}
          <button 
            onClick={(e) => { e.stopPropagation(); onRequestContact(asset.collateral_uuid); }}
...
export default function RepossedMarketplace({ setRole }) {
  ...
  const handleViewDetails = async (assetId) => {
    ...
    try {
        const accessToken = localStorage.getItem('accessToken');
        const response = await fetch(`${API_BASE_URL}/api/assets/repossessed_listings/${assetId}/`, {
            headers: { 'Authorization': `Bearer ${accessToken}` },
        });
  ...
  const handleContactOwner = async (asset) => {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/assets/repossessed_listings/${asset.collateral_uuid}/start-conversation/`, {
            method: 'POST',
            headers: { 
        ...
        navigate(`/buyer/conversations?convId=${data.id}`);
  ...
  const onRequestContact = (assetId) => handleViewDetails(assetId);
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 lg:ml-80 lg:mr-0 mr-20 transition-all duration-300">
          <div className="p-0 lg:p-4 w-full min-h-[85vh]">
            <header className="pb-6 mb-8 border-b border-gray-200">
              <h1 className="text-3xl font-bold text-gray-900">Repossessed Assets Marketplace</h1>
              <p className="mt-1 text-gray-600">Browse and find repossessed assets available for sale.</p>
            </header>
            ...
            {!loading && !error && (
              assets.length > 0 ? (
                <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {assets.map(asset => (
                    <div key={asset.collateral_uuid} onClick={() => handleViewDetails(asset.collateral_uuid)}>
                      <AssetCard asset={asset} onRequestContact={onRequestContact} />
                    </div>
                  ))}
                </div>
              ) : (
                <div className="text-center py-16 bg-white rounded-lg border border-gray-200">
                  ...
                  <p className="text-gray-600 mt-2">There are no repossessed assets listed for sale at the moment.</p>
                </div>
              )
            )}
          </div>
